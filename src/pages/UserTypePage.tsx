  ...
  return <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-background to-primary/5 p-4">
      <Card className="w-full max-w-sm">
        <CardHeader className="text-center space-y-2">
          ...
          <CardDescription className="text-base">Select your log in portal            </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-4 p-6">
          {userTypes.map(type => {
          const Icon = type.icon;
          return <Button key={type.title} variant={type.variant} className="h-auto flex-col gap-3 p-6 hover:scale-105 transition-transform" onClick={() => navigate(type.path)}>
