...
export const AllQuestionsStep = ({
  ...
  const renderScoreInput = (field: JsonField) => {
    ...
    switch (field.type) {
      case "label":
        return <div className="space-y-2 text-center py-2">
      ...
      case "penalty_checkbox":
        ...
        return <div className="space-y-4">
            ...
            <div className="flex gap-2 justify-center">
              <Button type="button" variant="outline" size="lg" onClick={handleDecrement} className="h-8 w-8 p-0">
                <Minus className="h-4 w-4" />
              </Button>
              <Input type="number" inputMode="numeric" pattern="[0-9]*" value={localValue ?? ""} onChange={e => handleValueChange(e.target.value)} placeholder="0" className="h-8 w-16 text-center text-base" />
              <Button type="button" variant="outline" size="lg" onClick={handleIncrement} className="h-8 w-8 p-0">
        ...
        return <Collapsible key={field.id} open={isExpanded} onOpenChange={() => toggleQuestion(field.id)}>
              <Card ref={el => questionRefs.current[field.id] = el} className={cn("overflow-hidden transition-all", isAnswered && "border-primary/50 bg-primary/5")}>
                <CollapsibleTrigger className="w-full">
                  <div className="p-4 flex items-center justify-between gap-3 py-[6px]">
                ...
                <CollapsibleContent>
                  <div className="pb-4 pt-2 border-t space-y-4 py-[4px] px-[4px]">
