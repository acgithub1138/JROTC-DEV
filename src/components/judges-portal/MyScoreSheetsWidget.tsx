...
export const MyScoreSheetsWidget = () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (scoreSheetId: string) => {
      const { error } = await supabase.from("competition_events").delete().eq("id", scoreSheetId);
  ...
  if (error) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-destructive">Error loading score sheets: {error.message}</p>
        </CardContent>
      </Card>
    );
  }
  if (scoreSheets.length === 0) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">No score sheets submitted yet</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        {scoreSheets.slice(0, 5).map((sheet) => (
          <div key={sheet.id} className="p-3 rounded-lg border bg-card hover:bg-accent/50 transition-colors">
            <div className="space-y-2">
              ...
              <div className="flex flex-col gap-1 text-xs text-muted-foreground">
                <div className="flex items-center gap-2">
                  ...
                  <span>{convertToUI(sheet.competition_start_date, timezone, "date")}</span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <Button
                    size="icon"
                    variant="outline"
                    className="h-7 w-7 text-red-600 hover:text-red-700 hover:border-red-300"
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete the score sheet for <strong>{sheetToDelete?.event_name}</strong>? This
              action cannot be undone and all scoring data will be permanently removed.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
