...
export const CadetBulkUploadPage: React.FC = () => {
  ...
  const { handleBulkImport } = useCadetManagement();
  ...
  const handleImport = async () => {
    ...
    const results = await handleBulkImport(validCadets, (current, total) => {
      ...
      <div className="mb-6">
        <div className="flex items-center gap-4 mb-4">
          <Button variant="ghost" onClick={handleBack} size="sm">
            ...
            {step === 'complete' ? 'Back to Cadets' : 'Back'}
          </Button>
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Bulk Import Cadets</h1>
            <p className="text-muted-foreground">
              Upload a CSV file to import multiple cadets
            </p>
          </div>
        </div>
      </div>
                  ...
                  <TableBody>
                    {parsedCadets.map(cadet => (
                      <TableRow key={cadet.id} className={!cadet.isValid ? 'bg-red-50' : ''}>
                        ...
                        <TableCell>
                          {editingRow === cadet.id ? (
                            <Select value={cadet.role_id} onValueChange={value => updateCadet(cadet.id, 'role_id', value)}>
                          ...
                          ) : roleOptions.find(r => r.value === cadet.role_id)?.label || cadet.role_id}
                        </TableCell>
                        <TableCell>
                          {editingRow === cadet.id ? (
                            <Select value={cadet.grade || ''} onValueChange={value => updateCadet(cadet.id, 'grade', value)}>
                              <SelectTrigger className="h-8">
                        ...
                        <TableCell>
                          {editingRow === cadet.id ? (
                            ...
                            >
                              <SelectTrigger className="h-8">
                                <SelectValue
                                  placeholder={ranks.length === 0 ? 
                                    userProfile?.schools?.jrotc_program ? "No ranks available" : "Set JROTC program first" 
                                    : "Select rank"
                                  }
                                />
                              </SelectTrigger>
                        ...
                        <TableCell>
                          {editingRow === cadet.id ? (
                            <Select value={cadet.flight || ''} onValueChange={value => updateCadet(cadet.id, 'flight', value)}>
                              <SelectTrigger className="h-8">
                        ...
                        <TableCell>
                          <div className="flex gap-1">
                            {editingRow === cadet.id ? (
                              ...
                              <Button size="sm" variant="outline" onClick={() => setEditingRow(cadet.id)} className="h-8 w-8 p-0">
