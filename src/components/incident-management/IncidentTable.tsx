...
export const IncidentTable: React.FC<IncidentTableProps> = ({
  ...
  const {
    ...
    canDelete,
    canViewDetails
  } = useModulePermissions('incidents');
  ...
  return <div className="bg-card rounded-lg border">
      <Table>
        <TableHeader>
          <TableRow>
        ...
        <TableBody>
          {incidents.map(incident => <TableRow key={incident.id}>
              <TableCell className="font-medium py-[8px]">
                <button onClick={() => canUpdate && canEditIncident(incident) ? onEditIncident(incident) : onIncidentSelect(incident)} className="text-blue-600 hover:text-blue-800 hover:underline cursor-pointer">
              ...
              <TableCell>
                {incident.submitted_by_profile ? `${incident.submitted_by_profile.first_name} ${incident.submitted_by_profile.last_name}` : 'Unknown'}
              </TableCell>
              <TableCell>
                {incident.assigned_to_profile ? `${incident.assigned_to_profile.first_name} ${incident.assigned_to_profile.last_name}` : 'Unassigned'}
              </TableCell>
              <TableCell>
                {new Date(incident.created_at).toLocaleDateString()}
              </TableCell>
              <TableCell className="text-right">
                <TableActionButtons canView={canViewDetails} canEdit={canUpdate && canEditIncident(incident)} canDelete={canDelete} onView={() => onIncidentSelect(incident)} onEdit={() => onEditIncident(incident)} customActions={[{
      ...
      {incidents.length === 0 && <div className="text-center py-8 text-gray-500">
