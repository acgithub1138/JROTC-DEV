...
export const IncidentFormContent: React.FC<IncidentFormContentProps> = ({
  ...
  const { uploadFile } = useAttachments('incident', incident?.id);
    ...
    if (mode === 'edit' && incident) {
      const subscription = form.watch((formData) => {
        const hasChanges = (
          ...
          formData.category !== incident.category ||
          (formData.due_date?.getTime() || null) !== (incident.due_date ? new Date(incident.due_date).getTime() : null)
        );
  ...
  const attachmentSection = showAttachments && incident ? (
    <AttachmentSection
      recordType="incident"
      recordId={incident.id}
      canEdit={canUpdate || (canUpdateAssigned && incident.assigned_to_admin === userProfile?.id)}
    />
  ) : null;
  ...
  return (
    <>
      <SharedIncidentFormLayout
        ...
        mode={mode}
        incident={incident}
        canAssignIncidents={canUpdate}
        priorityOptions={priorityOptions}
        categoryOptions={categoryOptions}
        titleField={<IncidentTitleField form={form} />}
        descriptionField={<IncidentDescriptionField form={form} />}
        attachmentSection={attachmentSection || fileInputSection}
        onCancel={handleCancel}
        submitButtonText={mode === 'create' ? 'Create Incident' : 'Save Changes'}
