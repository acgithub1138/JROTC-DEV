...
export const MobileHostSchedule: React.FC = () => {
  ...
  // Helper function to get event-specific time slots
  const getEventTimeSlots = (event: any) => {
    if (!timeline) return [];
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center space-x-4 text-xs text-muted-foreground mt-2">
                      ...
                      <div className="flex items-center">
                        ...
                        {(() => {
                          const timeSlots = getEventTimeSlots(event);
                          return `${timeSlots.filter(slot => slot.assignedSchool).length} / ${timeSlots.filter(slot => !slot.isLunchBreak).length} scheduled`;
                        })()}
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1 max-h-60 overflow-y-auto">
                    {getEventTimeSlots(event).map((slot, index) => (
                      <div key={index} className={`flex items-center justify-between p-2 rounded-md text-xs ${slot.isLunchBreak ? 'bg-orange-100 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800' : slot.assignedSchool ? 'bg-green-100 dark:bg-green-900/20 border border-green-200 dark:border-green-800' : 'bg-muted border border-border'}`}>
