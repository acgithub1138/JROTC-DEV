        ...
        <TableBody>
          {sortedTemplates.map(template => <TableRow key={template.id}>
              <TableCell className="font-medium py-2">
                {canEditTemplate?.(template) ? <button onClick={() => onEdit(template)} className="text-blue-600 hover:text-blue-800 hover:underline cursor-pointer bg-transparent border-none p-0 font-medium text-left">
                    {template.name}
