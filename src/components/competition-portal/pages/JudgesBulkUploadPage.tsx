...
export const JudgesBulkUploadPage: React.FC = () => {
  const navigate = useNavigate();
  const { bulkImportJudges } = useJudges();
  const { canCreate } = useCPJudgesPermissions();
  ...
  const handleImport = async () => {
    ...
    const results = await bulkImportJudges(validJudges, (current, total) => {
    ...
    return (
      <div className="p-6">
        <div className="flex items-center justify-center h-64">
          <div className="text-lg">You don't have permission to bulk upload judges.</div>
        </div>
      </div>
    );
  }
