...
export const AdvancedCriteriaMapping: React.FC<AdvancedCriteriaMappingProps> = ({
  ...
  const unmappedCriteria = availableCriteria.filter(criteria => !localMappings.some(mapping => mapping.original_criteria.includes(criteria)));
  ...
  const handleSaveEdit = async (mappingId: string) => {
    if (editingName.trim()) {
      const updatedMappings = localMappings.map(m => m.id === mappingId ? {
        ...m,
  ...
  const handleCriteriaToggle = (criteria: string) => {
    setSelectedCriteria(prev => {
      const newSelected = prev.includes(criteria) ? prev.filter(c => c !== criteria) : [...prev, criteria];
      ...
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
          ...
          <CardHeader className="cursor-pointer hover:bg-muted/50 transition-colors">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {localMappings.length > 0 && <Badge variant="secondary">{localMappings.length} mappings</Badge>}
        ...
        <CollapsibleContent>
          <CardContent className="space-y-6">
            {/* Create New Mapping */}
            {unmappedCriteria.length > 0 && <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex gap-2 items-end">
                    <div className="w-1/4">
                      ...
                      <Input id="mapping-name" value={newMappingName} onChange={e => setNewMappingName(e.target.value)} placeholder="e.g., Routine Marching" />
                    </div>
                    <Button onClick={handleCreateMapping} disabled={!newMappingName.trim() || selectedCriteria.length === 0 || isSaving}>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-2 max-h-40 overflow-y-auto">
                      {unmappedCriteria.map(criteria => <div key={criteria} className="flex items-center space-x-2">
                          <Checkbox id={`criteria-${criteria}`} checked={selectedCriteria.includes(criteria)} onCheckedChange={() => handleCriteriaToggle(criteria)} />
                          <Label htmlFor={`criteria-${criteria}`} className="text-sm cursor-pointer flex-1">
                            {criteria}
                          </Label>
                        </div>)}
                    </div>
            ...
            {/* Current Mappings */}
            {localMappings.length > 0 && <div className="space-y-3 border-t pt-4">
                <h4 className="font-medium">Current Mappings:</h4>
                {localMappings.map(mapping => <div key={mapping.id} className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex-1">
                      {editingMapping === mapping.id ? <div className="flex items-center gap-2">
                          <Input value={editingName} onChange={e => setEditingName(e.target.value)} className="max-w-xs" placeholder="Display name" autoFocus />
                    ...
                    {editingMapping !== mapping.id && <div className="flex items-center gap-2">
                        ...
                        <Button size="sm" variant="outline" onClick={() => handleDeleteMapping(mapping.id)} disabled={isSaving}>
            ...
            {unmappedCriteria.length === 0 && localMappings.length > 0 && <div className="text-center text-muted-foreground py-4">
                All criteria have been mapped!
              </div>}

            {unmappedCriteria.length === 0 && localMappings.length === 0 && <div className="text-center text-muted-foreground py-8">
