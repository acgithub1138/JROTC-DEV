...
BEGIN
  ...
  LOOP
    -- Skip nested objects (they're already handled above)
    IF key LIKE '%\_%' AND (key LIKE '%_first_name' OR key LIKE '%_last_name' OR key LIKE '%_email' OR key LIKE '%_name') THEN
      CONTINUE;
    END IF;
