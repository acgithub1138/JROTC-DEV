-- Drop the existing function explicitly (signature without defaults)
DROP FUNCTION IF EXISTS public.queue_email(uuid, text, text, uuid, uuid, uuid);

-- Recreate the function with explicit variables instead of a RECORD
...
BEGIN
  ...
  IF school_logo_url IS NOT NULL AND school_logo_url != '' THEN
    email_header_html := email_header_html || '<img src="' || school_logo_url || '" alt="' || COALESCE(school_name, 'School') || ' Logo" style="height: 60px; margin-bottom: 10px; vertical-align: middle;">';
  END IF;
  email_header_html := email_header_html || '<h1 style="margin: 0; color: #1e293b; font-family: -apple-system, BlinkMacSystemFont, ''Segoe UI'', Roboto, ''Helvetica Neue'', Arial, sans-serif; font-size: 24px; font-weight: 600;">' || COALESCE(school_name, 'School') || '</h1></div>';
