...
DROP POLICY IF EXISTS "profiles: restrict role updates" ON public.profiles;

-- Step 2: Now we can alter the column type
ALTER TABLE public.profiles ALTER COLUMN role TYPE text;

-- Step 3: Recreate the policy with text-based role checking
CREATE POLICY "profiles: restrict role updates" ON public.profiles
FOR UPDATE 
USING (
  auth.uid() = id OR 
  can_update_profile_role(id, NULL, role::text)
)
WITH CHECK (
  auth.uid() = id OR 
  can_update_profile_role(id, role_id, role::text)
);

-- Step 4: Update the handle_new_user function to work with text roles
