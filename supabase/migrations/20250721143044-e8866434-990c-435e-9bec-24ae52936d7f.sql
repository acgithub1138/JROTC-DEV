-- Update the validate_profile_role_change function to allow instructors to assign any role except admin/instructor
...
BEGIN
  ...
  IF OLD.role IS DISTINCT FROM NEW.role THEN
    ...
    -- Instructors can assign any role except admin or instructor within their school
    IF (SELECT public.get_current_user_role()) = 'instructor' AND
       OLD.school_id = (SELECT public.get_current_user_school_id()) AND
       NEW.role NOT IN ('admin', 'instructor') THEN
      RETURN NEW;
    END IF;
    
    -- Command staff can assign any role except admin or instructor within their school
    ...
    RAISE EXCEPTION 'Only administrators, users with Reset Password permission for cadets, or instructors/command staff can change user roles (instructors/command staff cannot assign admin or instructor roles)';
  END IF;
