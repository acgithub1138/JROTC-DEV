...
-- Add missing permission modules for tables that need them
...
-- Add missing permission actions if needed
INSERT INTO public.permission_actions (name) VALUES 
  ('manage')
ON CONFLICT (name) DO NOTHING;
...
-- Check if inventory_items table exists and update if it does
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'inventory_items') THEN
    DROP POLICY IF EXISTS "Instructors can manage inventory items in their school" ON public.inventory_items;
    EXECUTE 'CREATE POLICY "Users with inventory permissions can manage inventory items in their school"
...
-- Check if job_board table exists and update if it does
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'job_board') THEN
    DROP POLICY IF EXISTS "Instructors can manage job board in their school" ON public.job_board;
    EXECUTE 'CREATE POLICY "Users with job board permissions can manage job board in their school"
    ON public.job_board
    FOR ALL
    TO authenticated
    USING (
      school_id = public.get_current_user_school_id() 
      AND public.can_manage_job_board()
    )
    WITH CHECK (
      school_id = public.get_current_user_school_id() 
      AND public.can_manage_job_board()
...
-- Check if pt_tests table exists and update if it does
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'pt_tests') THEN
    DROP POLICY IF EXISTS "Instructors can manage pt tests in their school" ON public.pt_tests;
    EXECUTE 'CREATE POLICY "Users with appropriate permissions can manage pt tests in their school"
    ON public.pt_tests
    FOR ALL
    TO authenticated
    USING (
      school_id = public.get_current_user_school_id() 
      AND public.current_user_has_permission(''pt_tests'', ''update'')
    )
    WITH CHECK (
      school_id = public.get_current_user_school_id() 
      AND public.current_user_has_permission(''pt_tests'', ''update'')
...
-- Check if themes table exists and update if it does
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'themes') THEN
    DROP POLICY IF EXISTS "Instructors can manage themes in their school" ON public.themes;
    EXECUTE 'CREATE POLICY "Users with appropriate permissions can manage themes in their school"
    ON public.themes
    FOR ALL
    TO authenticated
    USING (
      school_id = public.get_current_user_school_id() 
      AND public.current_user_has_permission(''themes'', ''update'')
    )
    WITH CHECK (
      school_id = public.get_current_user_school_id() 
      AND public.current_user_has_permission(''themes'', ''update'')
