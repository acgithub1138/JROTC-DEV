-- Add a delay mechanism for competition registration emails to handle timing issues
-- This ensures event registrations are committed before the confirmation email is sent

-- Create a function to queue delayed competition registration emails
...
-- Disable the immediate email trigger temporarily
DROP TRIGGER IF EXISTS handle_comp_registration_email_trigger ON cp_comp_schools;

-- Create a new trigger that only creates a minimal log entry for tracking
CREATE OR REPLACE FUNCTION public.handle_comp_registration_created()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
AS $function$
BEGIN
  -- Just log that a registration was created
  -- The actual email will be sent by the delayed function
  RAISE LOG 'Competition registration created: registration_id=%, school_id=%, competition_id=%', 
    NEW.id, NEW.school_id, NEW.competition_id;
...
CREATE TRIGGER handle_comp_registration_created_trigger
  AFTER INSERT ON cp_comp_schools
  FOR EACH ROW
  EXECUTE FUNCTION handle_comp_registration_created();