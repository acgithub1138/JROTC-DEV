-- Update handle_new_user function to support role_id from metadata
...
BEGIN
  ...
  -- Extract role_id from metadata (if provided)
  BEGIN
    metadata_role_id := (NEW.raw_user_meta_data ->> 'role_id')::uuid;
  EXCEPTION WHEN OTHERS THEN
    metadata_role_id := NULL;
  END;
  ...
  -- Log role_id extraction
  IF metadata_role_id IS NOT NULL THEN
    RAISE LOG 'handle_new_user: Found role_id % for user %', metadata_role_id, NEW.id;
  END IF;
