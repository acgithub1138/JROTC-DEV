...
SELECT 
  COALESCE(cadet_stats.school_id, task_stats.school_id, budget_stats.school_id, inventory_stats.school_id, incident_stats.school_id) as school_id,
  COALESCE(cadet_stats.total_cadets, 0) as total_cadets,
  COALESCE(task_stats.active_tasks, 0) as active_tasks,
  COALESCE(task_stats.overdue_tasks, 0) as overdue_tasks,
  COALESCE(budget_stats.total_income, 0) as total_income,
  COALESCE(budget_stats.total_expenses, 0) as total_expenses,
  COALESCE(budget_stats.total_income, 0) - COALESCE(budget_stats.total_expenses, 0) as net_budget,
  COALESCE(inventory_stats.total_items, 0) as total_inventory,
  COALESCE(inventory_stats.total_issued, 0) as total_issued,
  COALESCE(inventory_stats.in_stock_count, 0) as in_stock_count,
  COALESCE(inventory_stats.out_of_stock_count, 0) as out_of_stock_count,
  COALESCE(incident_stats.active_incidents, 0) as active_incidents,
  COALESCE(incident_stats.overdue_incidents, 0) as overdue_incidents,
  COALESCE(incident_stats.urgent_critical_incidents, 0) as urgent_critical_incidents,
  COALESCE(SUM(community_service_stats.total_hours), 0) as community_service_hours,
  COALESCE(SUM(community_service_stats.total_records), 0) as community_service_records
FROM cadet_stats
FULL OUTER JOIN task_stats ON cadet_stats.school_id = task_stats.school_id
FULL OUTER JOIN budget_stats ON COALESCE(cadet_stats.school_id, task_stats.school_id) = budget_stats.school_id
FULL OUTER JOIN inventory_stats ON COALESCE(cadet_stats.school_id, task_stats.school_id, budget_stats.school_id) = inventory_stats.school_id
FULL OUTER JOIN incident_stats ON COALESCE(cadet_stats.school_id, task_stats.school_id, budget_stats.school_id, inventory_stats.school_id) = incident_stats.school_id
LEFT JOIN community_service_stats ON COALESCE(cadet_stats.school_id, task_stats.school_id, budget_stats.school_id, inventory_stats.school_id, incident_stats.school_id) = community_service_stats.school_id
GROUP BY 
  COALESCE(cadet_stats.school_id, task_stats.school_id, budget_stats.school_id, inventory_stats.school_id, incident_stats.school_id),
  cadet_stats.total_cadets, task_stats.active_tasks, task_stats.overdue_tasks,
  budget_stats.total_income, budget_stats.total_expenses, inventory_stats.total_items, inventory_stats.total_issued,
  inventory_stats.in_stock_count, inventory_stats.out_of_stock_count, incident_stats.active_incidents,
  incident_stats.overdue_incidents, incident_stats.urgent_critical_incidents;