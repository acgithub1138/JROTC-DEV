-- Update process_email_rules function to properly evaluate trigger_conditions
...
BEGIN
  ...
  FOR rule_record IN 
    ...
    WHERE er.source_table = TG_TABLE_NAME 
      AND er.trigger_event = TG_OP::email_trigger_event
  ...
  LOOP
    ...
    IF rule_record.trigger_conditions IS NOT NULL AND jsonb_array_length(rule_record.trigger_conditions) > 0 THEN
      ...
      LOOP
        ...
        -- Debug logging
        RAISE NOTICE 'Condition evaluation: field=%, value=%, expected=%, operator=%, result=%, overall=%', 
          condition_group->>'field', field_value, expected_value, operator_type, group_result, condition_met;
