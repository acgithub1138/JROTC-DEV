-- Test if the edge function is reachable with a simpler approach
-- Let's update the trigger to use PERFORM instead of SELECT and add more detailed logging
...
BEGIN
  ...
  IF NEW.status = 'pending' THEN
    BEGIN
      ...
      -- Use PERFORM instead of SELECT to avoid return value issues
      ...
      -- Also try calling the manual processor as a fallback
      PERFORM net.http_get(
        url := 'https://vpiwfabbzaebfkadmmgd.supabase.co/functions/v1/manual-process-emails',
        headers := '{"Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZwaXdmYWJiemFlYmZrYWRtbWdkIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1MDcxMDIyOSwiZXhwIjoyMDY2Mjg2MjI5fQ.MdcIjFOevGAZXlPf5q6-XBqB2aVBdJyDwjNhA77YyGg"}'::jsonb
      );
      
      -- Log successful webhook trigger
      INSERT INTO public.webhook_trigger_log (email_id, status, error_message) 
      VALUES (NEW.id, 'success', 'Called both webhook and manual processor');
