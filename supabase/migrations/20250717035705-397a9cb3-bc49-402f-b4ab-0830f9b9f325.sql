...
-- Remove existing cron jobs that cause multiple processors (if they exist)
DO $$
BEGIN
  BEGIN
    PERFORM cron.unschedule('process-email-queue-auto');
  ...
  BEGIN
    PERFORM cron.unschedule('email-backup-retry-processor');
  EXCEPTION WHEN OTHERS THEN
    -- Job doesn't exist, ignore error
    NULL;
  END;
