...
-- Fix the database function variable conflict and standardize recipient field processing
...
BEGIN
  ...
  LOOP
    ...
    resolved_recipient_email := NULL;
    
    IF rule_record.recipient_config->>'recipient_type' = 'field' THEN
      -- Process recipient field as a template variable (same as email templates)
      resolved_recipient_email := public.process_email_template(
        '{{' || (rule_record.recipient_config->>'recipient_field') || '}}', 
        record_data
      );
    ELSIF rule_record.recipient_config->>'recipient_type' = 'static' THEN
      -- Use static email
      resolved_recipient_email := rule_record.recipient_config->>'static_email';
    END IF;
    
    -- Skip if no valid recipient email
    IF resolved_recipient_email IS NULL OR resolved_recipient_email = '' THEN
      CONTINUE;
    END IF;
    ...
    WHERE eq.template_id = rule_record.template_id
      ...
      AND eq.recipient_email = resolved_recipient_email
