-- Fix the process_email_rules function - use correct field names
...
BEGIN
  ...
  FOR rule_record IN 
    SELECT er.*, et.subject, et.body, et.name as template_name, et.recipient_field
  ...
  LOOP
    ...
    IF should_send_email THEN
      ...
      -- Determine recipient email using the recipient_field from the template
      CASE rule_record.recipient_field
        ...
        WHEN 'school_admin' THEN
          ...
          LIMIT 1;
        ELSE
          -- Default to assigned_to email if recipient_field is not recognized
          recipient_email := (flattened_data->>'assigned_to_email')::text;
      END CASE;
      ...
      ELSE
        RAISE LOG 'Skipping email for rule type: % - no valid recipient email found (recipient_field: %, email: %)', 
          rule_record.rule_type, rule_record.recipient_field, recipient_email;
      END IF;
    END IF;
  END LOOP;
