...
-- Add constraint to ensure cadet belongs to the same school
ALTER TABLE public.pt_tests
ADD CONSTRAINT pt_tests_cadet_school_check
CHECK (
  EXISTS (
    SELECT 1 FROM public.profiles 
    WHERE profiles.id = pt_tests.cadet_id 
    AND profiles.school_id = pt_tests.school_id
  )
);