-- Fix both constraints to allow the new rule type and trigger event
-- Drop the existing constraints
ALTER TABLE public.email_rules DROP CONSTRAINT IF EXISTS email_rules_rule_type_check;
ALTER TABLE public.email_rules DROP CONSTRAINT IF EXISTS email_rules_trigger_event_check;

-- Add the new constraints that include our new values
...
ALTER TABLE public.email_rules ADD CONSTRAINT email_rules_trigger_event_check 
CHECK (trigger_event IN ('INSERT', 'UPDATE', 'SCHEDULED'));

-- Now add the overdue reminder rules for all schools
DO $$
DECLARE
