-- Update queue_email to include last_comment for tasks and subtasks
...
DECLARE
  ...
  task_data RECORD;
  subtask_data RECORD;
  last_comment_text TEXT; -- NEW: holds the latest comment text
BEGIN
  -- Get the template
  ...
  IF source_table_param = 'tasks' THEN
    ...
    -- NEW: get the latest non-system comment, fallback to any comment
    ...
    IF FOUND THEN
      record_data := jsonb_build_object(
        ...
        'last_comment', COALESCE(last_comment_text, '') -- NEW
      );
    END IF;
  ...
  ELSIF source_table_param = 'subtasks' THEN
    ...
    -- NEW: get the latest non-system comment for subtask, fallback to any comment
    last_comment_text := NULL;
