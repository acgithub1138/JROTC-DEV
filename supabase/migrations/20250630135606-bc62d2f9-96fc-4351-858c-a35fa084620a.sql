...
-- Drop existing function and recreate with enhanced capabilities
DROP FUNCTION IF EXISTS execute_business_rules() CASCADE;

-- Create enhanced business rule execution function
            ...
            LOOP
                ...
                IF TG_TABLE_NAME = 'tasks' THEN
                    CASE condition_item->>'field'
                        WHEN 'status' THEN 
                            field_value := CASE WHEN TG_OP = 'DELETE' THEN OLD.status::text ELSE NEW.status::text END;
                        WHEN 'priority' THEN 
                            field_value := CASE WHEN TG_OP = 'DELETE' THEN OLD.priority::text ELSE NEW.priority::text END;
                ...
                CASE action->>'type'
                    WHEN 'update_record' THEN
                        ...
                        IF TG_TABLE_NAME = 'tasks' AND TG_OP != 'DELETE' THEN
                            -- Update task status
                            IF action->'parameters'->>'field' = 'status' THEN
                                UPDATE tasks 
                                SET status = (action->'parameters'->>'value')::task_status,
                                    updated_at = now()
                                WHERE id = NEW.id;
                            ...
                            IF action->'parameters'->>'field' = 'priority' THEN
                                UPDATE tasks 
                                SET priority = (action->'parameters'->>'value')::task_priority,
                                    updated_at = now()
                                WHERE id = NEW.id;
                            ...
                            IF action->'parameters'->>'field' = 'assigned_to' THEN
                                UPDATE tasks 
