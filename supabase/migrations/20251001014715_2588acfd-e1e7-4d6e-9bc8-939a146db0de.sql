-- Update the attachment tracking trigger to support event attachments
-- Events don't have a comment table, so they skip comment tracking like budget_transactions
...
DECLARE
  ...
  user_profile RECORD;
BEGIN
  -- Only track attachments for tasks, subtasks, events, and budget_transactions
  IF (TG_OP = 'INSERT' AND NEW.record_type NOT IN ('task', 'subtask', 'incident', 'budget_transaction', 'event')) OR 
     (TG_OP = 'DELETE' AND OLD.record_type NOT IN ('task', 'subtask', 'incident', 'budget_transaction', 'event')) THEN
    RETURN COALESCE(NEW, OLD);
  END IF;

  -- Skip comment tracking for budget_transaction and event (no comment tables)
  IF (TG_OP = 'INSERT' AND NEW.record_type IN ('budget_transaction', 'event')) OR 
     (TG_OP = 'DELETE' AND OLD.record_type IN ('budget_transaction', 'event')) THEN
    RETURN COALESCE(NEW, OLD);
  END IF;
