-- Add specific permission actions for granular control
...
('sidebar', 'Show in Sidebar', 'Can see module in sidebar navigation');
...
WHERE pa.name IN ('assign', 'activate_deactivate', 'bulk_import', 'manage_templates', 'manage_hierarchy', 'manage_options', 'reset_password', 'view_analytics', 'submit', 'approve', 'manage_scoring', 'sidebar');
...
SELECT 
  ...
  CASE 
    ...
    WHEN pa.name = 'sidebar' THEN true
    ELSE false
  END
FROM permission_modules pm
CROSS JOIN permission_actions pa
...
-- Update default role permissions for cadet role
INSERT INTO default_role_permissions (role, module_id, action_id, enabled) 
SELECT 
  'cadet'::user_role,
  pm.id,
  pa.id,
  CASE 
    WHEN pa.name = 'sidebar' AND pm.name IN ('tasks', 'events', 'competitions', 'job_board') THEN true
    ELSE false
  END
FROM permission_modules pm
CROSS JOIN permission_actions pa
