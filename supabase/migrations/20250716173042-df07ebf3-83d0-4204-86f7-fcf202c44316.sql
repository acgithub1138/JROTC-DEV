-- Create a function to resolve user email with job board priority (same logic as useEmailResolution hook)
CREATE OR REPLACE FUNCTION public.resolve_user_email_with_job_priority(user_id_param UUID, school_id_param UUID)
RETURNS TABLE(email TEXT, source TEXT)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
AS $$
DECLARE
  user_jobs_count INTEGER;
  job_emails_data RECORD;
  all_jobs_data RECORD;
  highest_priority_job RECORD;
  lowest_level INTEGER := 999999;
  profile_email_result TEXT;
BEGIN
  -- First, get count of job assignments for this user with email addresses
  SELECT COUNT(*) INTO user_jobs_count
  FROM public.job_board 
  WHERE cadet_id = user_id_param 
    AND school_id = school_id_param
    AND email_address IS NOT NULL 
    AND email_address != '';

  -- If user has job assignments with emails, determine priority based on hierarchy
  IF user_jobs_count > 0 THEN
    -- If only one job, use that email
    IF user_jobs_count = 1 THEN
      SELECT jb.email_address INTO profile_email_result
      FROM public.job_board jb
      WHERE jb.cadet_id = user_id_param 
        AND jb.school_id = school_id_param
        AND jb.email_address IS NOT NULL 
        AND jb.email_address != ''
      LIMIT 1;
      
      RETURN QUERY SELECT profile_email_result, 'job_board'::TEXT;
      RETURN;
    END IF;

    -- Multiple jobs - need to determine hierarchy priority
    -- For now, we'll use the first job with email as a fallback
    -- TODO: Implement full hierarchy logic when needed
    SELECT jb.email_address INTO profile_email_result
    FROM public.job_board jb
    WHERE jb.cadet_id = user_id_param 
      AND jb.school_id = school_id_param
      AND jb.email_address IS NOT NULL 
      AND jb.email_address != ''
    ORDER BY jb.created_at ASC
    LIMIT 1;
    
    IF profile_email_result IS NOT NULL THEN
      RETURN QUERY SELECT profile_email_result, 'job_board'::TEXT;
      RETURN;
    END IF;
  END IF;

  -- No job assignments with emails, fall back to profile email
  SELECT p.email INTO profile_email_result
  FROM public.profiles p 
  WHERE p.id = user_id_param;

  IF profile_email_result IS NOT NULL THEN
    RETURN QUERY SELECT profile_email_result, 'profile'::TEXT;
  ELSE
    RETURN QUERY SELECT NULL::TEXT, NULL::TEXT;
  END IF;
END;
$$;

-- Update the process_email_rules function to use the new email resolution
CREATE OR REPLACE FUNCTION public.process_email_rules()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $$
DECLARE
  email_rule RECORD;
  recipient_email TEXT;
  email_source TEXT;
  rule_type_to_check TEXT;
  old_status TEXT;
  new_status TEXT;
  start_time TIMESTAMP;
  end_time TIMESTAMP;
  processing_time_ms INTEGER;
  queue_id UUID;
  email_result RECORD;
BEGIN
  start_time := clock_timestamp();
  
  -- Only process INSERT and UPDATE events
  IF TG_OP NOT IN ('INSERT', 'UPDATE') THEN
    RETURN COALESCE(NEW, OLD);
  END IF;

  -- For tasks table, determine rule type based on operation and status changes
  IF TG_TABLE_NAME = 'tasks' THEN
    IF TG_OP = 'INSERT' THEN
      rule_type_to_check := 'task_created';
    ELSIF TG_OP = 'UPDATE' THEN
      old_status := OLD.status;
      new_status := NEW.status;
      
      -- Check if status actually changed
      IF old_status IS DISTINCT FROM new_status THEN
        -- Map status to rule type
        CASE new_status
          WHEN 'completed' THEN rule_type_to_check := 'task_completed';
          WHEN 'canceled', 'cancelled' THEN rule_type_to_check := 'task_canceled';
          WHEN 'information_needed' THEN rule_type_to_check := 'task_information_needed';
          ELSE rule_type_to_check := NULL;
        END CASE;
      ELSE
        -- Status didn't change, no email rules to process
        RETURN NEW;
      END IF;
    END IF;
  ELSE
    -- For other tables, default to created rule
    rule_type_to_check := TG_TABLE_NAME || '_created';
  END IF;

  -- Skip if no rule type determined
  IF rule_type_to_check IS NULL THEN
    RETURN COALESCE(NEW, OLD);
  END IF;

  -- Process each active email rule (optimized query with indexes)
  FOR email_rule IN 
    SELECT er.id, er.rule_type, er.template_id, et.subject, et.body, et.source_table
    FROM public.email_rules er
    JOIN public.email_templates et ON er.template_id = et.id
    WHERE er.school_id = COALESCE(NEW.school_id, OLD.school_id)
      AND er.rule_type = rule_type_to_check
      AND er.trigger_event = TG_OP
      AND er.is_active = true
      AND et.is_active = true
      AND et.source_table = TG_TABLE_NAME
  LOOP
    -- Determine recipient email based on rule type and task data using new resolution function
    recipient_email := NULL;
    email_source := NULL;
    
    IF TG_TABLE_NAME = 'tasks' THEN
      CASE email_rule.rule_type
        WHEN 'task_created' THEN
          -- Send to assigned person (if assigned) or the person who assigned it
          IF NEW.assigned_to IS NOT NULL THEN
            -- Use job board email resolution for assigned person
            SELECT email_res.email, email_res.source INTO recipient_email, email_source
            FROM public.resolve_user_email_with_job_priority(NEW.assigned_to, NEW.school_id) AS email_res(email, source);
          ELSIF NEW.assigned_by IS NOT NULL THEN
            -- Use job board email resolution for person who assigned
            SELECT email_res.email, email_res.source INTO recipient_email, email_source
            FROM public.resolve_user_email_with_job_priority(NEW.assigned_by, NEW.school_id) AS email_res(email, source);
          END IF;
          
        WHEN 'task_completed', 'task_canceled' THEN
          -- Send to the person who assigned the task
          IF NEW.assigned_by IS NOT NULL THEN
            SELECT email_res.email, email_res.source INTO recipient_email, email_source
            FROM public.resolve_user_email_with_job_priority(NEW.assigned_by, NEW.school_id) AS email_res(email, source);
          END IF;
          
        WHEN 'task_information_needed' THEN
          -- Send to the assigned person
          IF NEW.assigned_to IS NOT NULL THEN
            SELECT email_res.email, email_res.source INTO recipient_email, email_source
            FROM public.resolve_user_email_with_job_priority(NEW.assigned_to, NEW.school_id) AS email_res(email, source);
          END IF;
      END CASE;
    END IF;

    -- Only proceed if we have a recipient email
    IF recipient_email IS NOT NULL AND recipient_email != '' THEN
      BEGIN
        -- Queue the email using the existing queue_email function
        SELECT public.queue_email(
          email_rule.template_id,
          recipient_email,
          TG_TABLE_NAME,
          NEW.id,
          COALESCE(NEW.school_id, OLD.school_id)
        ) INTO queue_id;
        
        -- Calculate processing time
        end_time := clock_timestamp();
        processing_time_ms := EXTRACT(MILLISECONDS FROM (end_time - start_time))::INTEGER;
        
        -- Log successful rule usage with email source information
        PERFORM public.log_email_rule_usage(
          email_rule.id,
          TG_TABLE_NAME,
          TG_OP,
          NEW.id,
          recipient_email,
          true,
          'Email resolved from: ' || COALESCE(email_source, 'unknown'),
          processing_time_ms,
          COALESCE(NEW.school_id, OLD.school_id)
        );
        
        -- Add a system comment to the task indicating email was queued
        IF TG_TABLE_NAME = 'tasks' THEN
          INSERT INTO public.task_comments (
            task_id,
            user_id,
            comment_text,
            is_system_comment
          ) VALUES (
            NEW.id,
            COALESCE(auth.uid(), NEW.assigned_by, NEW.created_by),
            'Email notification sent: ' || email_rule.rule_type || ' to ' || recipient_email || ' (source: ' || COALESCE(email_source, 'unknown') || ')',
            true
          );
        END IF;
        
      EXCEPTION WHEN OTHERS THEN
        -- Calculate processing time for failed operation
        end_time := clock_timestamp();
        processing_time_ms := EXTRACT(MILLISECONDS FROM (end_time - start_time))::INTEGER;
        
        -- Log failed rule usage
        PERFORM public.log_email_rule_usage(
          email_rule.id,
          TG_TABLE_NAME,
          TG_OP,
          NEW.id,
          COALESCE(recipient_email, 'unknown'),
          false,
          'Email resolution failed: ' || SQLERRM,
          processing_time_ms,
          COALESCE(NEW.school_id, OLD.school_id)
        );
        
        -- Log error but don't fail the main operation
        RAISE WARNING 'Failed to queue email for rule %: %', email_rule.id, SQLERRM;
      END;
    ELSE
      -- Log when no email could be resolved
      PERFORM public.log_email_rule_usage(
        email_rule.id,
        TG_TABLE_NAME,
        TG_OP,
        NEW.id,
        'no-email-found',
        false,
        'No email address could be resolved for user',
        NULL,
        COALESCE(NEW.school_id, OLD.school_id)
      );
    END IF;
  END LOOP;

  RETURN COALESCE(NEW, OLD);
END;
$$;