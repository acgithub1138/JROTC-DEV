-- Fix the duplicate queue_email function issue
-- Drop any existing queue_email functions and recreate the correct one
DROP FUNCTION IF EXISTS public.queue_email(uuid, text, text, uuid, uuid);
DROP FUNCTION IF EXISTS public.queue_email(uuid, text, text, uuid, uuid, uuid);

-- Recreate the correct queue_email function with proper signature
...
BEGIN
  ...
  -- For profiles table (welcome emails), create simple record data
  IF source_table_param = 'profiles' THEN
    SELECT to_jsonb(profiles.*) INTO record_data
